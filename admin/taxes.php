...
$filing_year = isset($_GET['year']) ? $_GET['year'] : date("Y") ;

// pagination
$page = ((isset($_GET['page'])) && (preg_match("/[0-9]+$/",$_GET['page']))) ? $_GET['page'] : 0;
$show = 1000; //((isset($_GET['show'])) && (preg_match("/[0-9]+$/",$_GET['show']))) ? $_GET['show'] : $invoicerpp;
...
switch ($search_type) {
	case 'invoice':
        $subq = "i.issue_date LIKE '%$filing_year%' AND (curr_status like '%fully paid%' OR curr_status like '%pending%')";
